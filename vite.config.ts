...
/**
 * Emits dist/_redirects and dist/vercel.json so the SPA host forwards
 * `/api/v1/*` (management API) and `/v1/*` (AI gateway) calls to a real
 * sub2api backend instead of falling back to index.html (which would
 * return the SPA's HTML for what should be a JSON response — exactly
 * the "404 not found" the user hit).
 *
 * Two scenarios:
 *
 *   1. Backend embedded in the same process (sub2api built with
 *      `-tags embed`): no rewrite needed; sub2api itself routes
 *      everything. Skip this plugin by not setting BACKEND_URL.
 *
 *   2. Frontend on a static host (Cloudflare Pages / Netlify / Vercel),
 *      backend on a separate URL: set BACKEND_URL at build time. The
 *      plugin then emits the right config for each host so /api/v1/*
 *      and /v1/* tunnel to the backend transparently.
 *
 * BACKEND_URL is read from the build env: `BACKEND_URL=https://api.your
 * .example.com pnpm build`. Cloudflare Pages / Netlify / Vercel all let
 * you set this as an environment variable in the build settings.
 */
function emitHostingRewrites(): PluginOption {
  return {
    name: 'amodel:emit-rewrites',
    apply: 'build',
    generateBundle() {
      const raw = process.env.BACKEND_URL ?? process.env.VITE_BACKEND_URL ?? ''
      const root = raw.trim().replace(/\/+$/, '')

      // Cloudflare Pages and Netlify both read this file. Comments are
      // supported. Rules at top take precedence; the SPA catch-all stays
      // last so client-side routing still works.
      const redirects = root
        ? `# Generated by Vite build. BACKEND_URL=${root}
# DO NOT edit by hand — set BACKEND_URL at build time.
/api/v1/*  ${root}/api/v1/:splat  200
/v1/*      ${root}/v1/:splat      200
# SPA fallback for client-side router. KEEP LAST.
/*         /index.html             200
`
        : `# No BACKEND_URL set at build time.
# Add your sub2api backend below so the SPA can reach the management API
# and the AI gateway. Without these rules, /api/v1/* and /v1/* will fall
# through to the SPA and return HTML instead of JSON, breaking login,
# key management, and every SDK call.
#
# /api/v1/*  https://YOUR-BACKEND.example.com/api/v1/:splat  200
# /v1/*      https://YOUR-BACKEND.example.com/v1/:splat      200
#
...
`
      ...
      // Vercel reads vercel.json. Same intent expressed in a different
      // schema. The headers section duplicates public/_headers because
      // vercel.json overrides whatever Vercel auto-generates from
      // _headers.
      const vercelJson = {
        ...
        rewrites: root
          ? [
              ...
              { source: '/v1/:path*', destination: `${root}/v1/:path*` },
            ]
          : [],
        headers: [
          { source: '/(sw|workbox-.*)\\.js', headers: [{ key: 'Cache-Control', value: 'public, max-age=0, must-revalidate' }] },
          { source: '/(version\\.json|recover\\.html)', headers: [{ key: 'Cache-Control', value: 'no-store, no-cache, must-revalidate' }] },
          { source: '/assets/(.*)', headers: [{ key: 'Cache-Control', value: 'public, max-age=31536000, immutable' }] },
        ],
      }
      this.emitFile({ type: 'asset', fileName: 'vercel.json', source: JSON.stringify(vercelJson, null, 2) })
    },
  }
}
...
export default defineConfig({
  plugins: [
    ...
    tailwindcss(),
    pwaPlugin,
    !isStorybook && emitVersionJson(),
    !isStorybook && emitHostingRewrites(),
  ],
  ...
  server: {
    ...
    host: '0.0.0.0',
    proxy: {
      // Management API used by the SPA itself.
      '/api/v1': {
        target: process.env.BACKEND_URL || 'https://sub2api.gomami.wiki',
        changeOrigin: true,
        secure: false,
      },
      // AI gateway path. Required so curl / SDK demos against
      // `http://localhost:5174/v1/...` reach the real upstream during
      // dev, instead of being claimed by the SPA's catch-all router.
      '/v1': {
        target: process.env.BACKEND_URL || 'https://sub2api.gomami.wiki',
        changeOrigin: true,
        secure: false,
